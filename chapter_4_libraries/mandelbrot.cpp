// From: https://github.com/petergottschling/discovering_modern_cpp/blob/master/c%2B%2B11/mandelbrot.cpp
...
{
public:
    julia_pixel(SDL_Surface* screen, int x, int y, int xdim, int ydim, int max_iter)
...
int main (int argc, char* argv[])
{
    ...
    for (int y= 0; y < ydim; y++)
        for (int x= 0; x < xdim; x++) {
            julia_pixel m(screen, x, y, xdim, ydim, max_iter);
            put_pixel(screen, x, y, m.color());
        }
    SDL_UnlockSurface(screen);
    SDL_Flip(screen);
    // Not sure why but outputing to bmp works but interactive session does not
